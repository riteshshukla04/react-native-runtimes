...
1. **`index.js` loads the generated entry.** It must require `.threaded-runtime/entry` unconditionally (in every runtime). See [Metro setup](/docs/getting-started/metro-setup).
...
## "Cannot create an instance of HybridObject" on Android
...
Shared paths are designed around a clear ownership model: one runtime writes
a path, everyone else reads and subscribes. That's what keeps writes fast and
predictable without cross-runtime locking. Give the contended path a single
owning runtime, and when another runtime needs to change it, send the request
to the owner as a
[runtime function](/docs/threaded-runtime/scheduling-functions) or
[headless task](/docs/threaded-runtime/headless-runtime) and let the owner
write. See
[Concurrency and revisions](/docs/shared-state/locking-and-revisions#concurrent-writers).
...
- The function's file must be under one of the Metro plugin's `roots`, so the generated entry registers it for the target runtime to load.
