...
- The main runtime renders UI and handles user interaction.
- A `background` runtime stays warm and owns sync, caching, queues, parsing, and other non-visual work.
...
## Getting results back to main

Don't schedule functions *onto* the main runtime — write results to the
shared store from the background runtime and let main subscribe:

```tsx
// Background runtime writes:
await business.set(snapshot, true);

// Main runtime UI re-renders automatically:
function BusinessStatus() {
  const state = business.use();
  return <Text>{state.lastRefreshReason ?? 'idle'}</Text>;
}
...
For request/response work initiated by main, the return value of an awaited
runtime function already comes back to the caller — no extra scheduling step
is needed.
...
Directive functions must be declared at module scope so Metro can rewrite and
register them before calls are made.
