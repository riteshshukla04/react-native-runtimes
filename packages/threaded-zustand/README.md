...
The Android implementation stores serialized state in a process-wide C++
singleton. The main runtime and any secondary React runtime register the same
native module, so either side can read or update the same store by name.
